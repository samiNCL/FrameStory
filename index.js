...
function fetchReflectionsForVideo(videoId) {
    ...
    console.log(`Fetching reflections for videoId: ${videoId}`);

    // Use the parameter name expected by the backend (e.g., 'video_id')
    fetch(`http://127.0.0.1:8000/api/resources?video_id=${videoId}`, {
        method: "GET",
        headers: {
...
function displayReflections(data) {
    if (data && data.resources && data.resources.length > 0) {
        console.log("Number of resources:", data.resources.length);
        console.log("First resource:", data.resources[0]);

        // Find the resource that matches the current videoId
        const matchingResource = data.resources.find(resource => {
            // Assuming resource.text contains the video URL
            const resourceVideoId = getYouTubeVideoId(resource.text);
            console.log(`Checking resource with videoId: ${resourceVideoId}`);
            return resourceVideoId === videoId;
        });
        ...
        if (matchingResource) {
            ...
            const { reflection, rating, tag } = matchingResource;

            // Log the raw reflections data
            ...
            // Ensure fetchedReflections is an array
